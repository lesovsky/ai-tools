---
name: dev-userspec-adequacy-validator
description: |
  ...
  acceptance criteria wording) — that is dev-userspec-quality-validator's scope.
...
Evaluate adequacy and feasibility of user-spec at the provided path.

This agent assesses the idea itself — is the proposed solution reasonable and buildable? Document quality (template compliance, section completeness, acceptance criteria wording) is handled by `dev-userspec-quality-validator`.

## Input

- `feature_base`: path prefix for feature artifacts (e.g., `docs/features/001-feat-add-auth/001-feat-add-auth`)
- `report_path`: where to write JSON report (e.g., `docs/features/001-feat-add-auth/001-feat-add-auth-adequacy-review.json`)
...
└── 001-feat-add-auth/
    ...
    ├── 001-feat-add-auth-code-research.md          # code research (if exists)
    └── 001-feat-add-auth-adequacy-review.json      # this agent's output
```

## Process

1. Read `{feature_base}.md` (user-spec)
2. Read `{feature_base}-code-research.md` (if exists)
...
- **Stack built-ins**: a standard solution (built-in middleware, library function, framework feature) exists instead of custom implementation
