---
name: code-writing
description: Универсальный процесс написания кода — план, TDD, ревью. Используй всегда когда нужно писать код.
---
...
2. **Read Project Context (Graceful)**
   ...
   **Working on a task?** Read all files listed in the task's "Context Files" section — it already specifies everything needed.
   ...
   **Standalone (no task file)?** Read (skip if missing):
   ...
   Then read `.claude/skills/project-knowledge/SKILL.md` (if exists) to find domain-specific guides.
   Consider which are relevant to your task (e.g., `architecture.md` Data Model section for DB work, `ux-guidelines.md` for UI tasks).
...
3. **Run Reviews** (launch in parallel)
   ...
   **Reviewer selection:**
   ...
   - Standalone (no task file) → default: `dev-code-reviewer`, `dev-security-auditor`, `dev-test-reviewer`

   For each reviewer:
   1. Spawn subagent via Task tool (subagent_type = reviewer name, e.g. `dev-code-reviewer`)
   2. Pass: git diff of changes, path to task file (if any), path to tech-spec, path to user-spec
   3. Reviewer loads its own skill automatically (via agent frontmatter `skills:`)
   4. Report path: from the task's "Reviewers" section, or `logs/reviews/{reviewer-name}-{round}.json` if standalone
...
4. **Process Findings**
   ...
   For each finding:
   - **Valid, improves code** → apply (any severity: critical, major, minor)
