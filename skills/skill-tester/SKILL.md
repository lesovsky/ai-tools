...
When all 3 runners finish, spawn grader agents (one per runner) that read
transcripts and return structured evaluations. Graders work in isolation —
transcripts are too large to read directly.
...
You now have 3 structured evaluations (one per runner). Using only the
grader outputs:
